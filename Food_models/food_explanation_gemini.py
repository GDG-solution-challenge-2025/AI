...
def explain_food_gemini(food_name, exp_type, food_info, food_list):
    ...
    if exp_type == '어떤 맛인지':
        prompt = f"""
        '{food_name}'이 무슨 맛인지 알려줘. 간단하게 2문장 정도로 설명해줘.
        """
        #What does '{food_name}' taste like? Please answer briefly in 2-3 sentences.
        
    elif exp_type == '유래':
        prompt = f"""
        '{food_name}'의 유래가 뭐야? 간단하게 2문장 정도로 설명해줘. 만약 확실하지 않다면 그냥 '유래가 확실하지 않습니다'라고 답해줘.
        """
        ...
        prompt = f"""
        '{food_name}'은 일반적으로 어떻게 먹어?(e.g., 밥과 함께, 국처럼, 반찬처럼 ..). 레시피를 알려달라는게 아니야. 답변은 2~3문장정도로 간단한 문장으로 설명해줘.
        """
        ...
        prompt = f"""
        '{food_name}'에 들어가는 일반적인 재료를 설명해줘. 추가 설명 필요없고 재로들은 각각 엔터로 구분해서 줘.
        """
        ...
        제한 항목과 일치하는 재료(또는 그 기본 구성 요소, 예: 두부 → 콩)를 확인하고 나열하세요.
